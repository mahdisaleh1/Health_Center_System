...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO specialty (specialty_name, keywords) VALUES (?, ?)";
    $stmt = $con->prepare($sql);
    $stmt->bind_param("ss", $specialty_name, $keywords);
    if ($stmt->execute()) {
        echo '<script>alert("Specialty has been added!" );</script>';
        header("Refresh:0.11; url=./doctorspecialty.php");
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Doctor Specialties - Admin </title>
...
<body id="top" data-spy="scroll" data-target=".navbar-collapse" data-offset="50">
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse">
                <ul class="nav navbar-nav navbar-right">
                    ...
                    <li class="lidashboard"><a href="./doctorspecialty.php" class="smoothScroll">Doctor's specialty</a></li>
    ...
    <div class="dashcontainer">
        <aside class="sidebar">
            <nav>
                <ul>
                    ...
                    <li class="lidashboard"><a href="./doctorspecialty.php">Doctor's specialty</a></li>
        ...
        <main class="content">
            <header>
                <p>You can Create and Modify Specialties! Specialties cannot be deleted! (active -> visible)</p>
            </header>
            <form action="./doctorspecialty.php" method="post">
                <div class="doctorspecialty">
                    <h3>Create New Specialty</h3><br>
                    <label>Specialty Name:</label>
                    <input type="text" name="specialtyname" id="specialty" placeholder="Enter Specialty Name" required>
                    <label>Specialty Keywords:</label>
                    <textarea name="keywords" placeholder="Enter specialty keywords e.g.(chest, blood, fever)"></textarea>
                    <button type="submit" name="submit">Add Specialty</button>
                </div><br><br>
                <div class="specialtyTable">
                    <h3>Check Available Specialties</h3><br>
                        ...
                        <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo "<tr>
                                    ...
                                    <td>{$row['keywords']}</td>
                                    <td class='status-cell' data-id='{$row['specialty_id']}' data-status='{$row['status']}'>{$row['status']}</td>
                                    <td><a class='updateactivation' data-id='" . $row['specialty_id'] . "' data-status='" . $row['status'] . "'><i class='fa fa-edit'></i></a></td>
                                </tr>";
                            }
                            ?>
                        </tbody>
...
<script>
    ...
    `;
        ...
        $('.statusDropdown').change(function() {
            ...
            $.ajax({
                url: './function_specialty/update_status.php',
                type: 'POST',
                data: {
                    id: userId,
                    status: newStatus
...
<style>
    ...
    .doctorspecialty input[type="text"] {
        width: 100%;
        padding: 1.5rem;
