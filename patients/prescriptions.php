...
<head>
    ...
    <title>Prescriptions - Health Medical Center</title>
        ...
        <div class="dashcontainer">
            ...
            <main class="content">
                ...
                <section class="cards">
                    ...
                    WHERE patient_id = $patient_id ORDER BY created_at DESC");
                    while ($row = $result->fetch_assoc()) {
                        echo "<div class='card'>
                            <h3><strong>Doctor:</strong> {$row['doctor_name']}</h3>
                            <p><strong>Diagnosis:</strong> {$row['diagnosis']}</p>
                            <p><strong>Medications:</strong> {$row['medications']}</p>
                            <p><strong>Instructions:</strong> {$row['instructions']}</p>
                            <h6><strong>Date:</strong> {$row['date_issued']}</h6>
                            <a href='./presc_details.php?id={$row['pres_id']}'><button type='button'>Check Details</button></a>
                        </div>";
                    }
